...
Prerequisites: Python 3.9+, familiarity with lexers and parsers
...
Markup+ follows a classic compiler pipeline:
    ...
    Source Text       .mup file (or .md), as string
         |
         v
    ...
    (part of          Variables, filters, @if, @each
     renderer)
         |
         v
    Renderer          renderer.py -- AST to HTML
                      Output: HTML string
         |
         v
...
renderer.resolve_imports() replaces ImportBlock nodes with parsed
children of imported files (recursively).

### Step 4: Rendering

renderer.render_ast() walks the AST, evaluates conditions and loops,
and produces HTML:

    context = {"title": "Demo", "name": "Ali"}
    # Note: version is undefined, so condition "version > 1" is False
...
Node categories:
    ...
    Rich content      ChartBlock, MathBlock, TabsBlock,
                      CollapseBlock, AlertBlock, QuoteBlock,
                      TimelineBlock
    Imports           ImportBlock
...
(`**bold**`, backtick code) is parsed later by regex in the renderer.
...
Supports Persian (\u0600-\u06FF) and Arabic Supplement
(\u0750-\u077F).
...
_split_top_level() splits by comma, ignoring commas inside brackets
or quotes. Used for arrays and component arguments.

### 4.4 renderer.py -- Renderer

The largest module. Four parts.
...
Note: The evaluator is intentionally part of renderer.py. In a larger
codebase, it would be in evaluator.py. For Markup+, keeping it here
...
Threshold: 30 percent RTL characters -- document becomes RTL.
...
- Parses global flags (--dark, --rtl, -o, --debug, --css)
...
Walk through adding @badge step-by-step.
...
- CHANGELOG.md -- add under [Unreleased] then Added
...
Colors are auto-disabled when output is not a TTY or NO_COLOR env is
set.
...
1. Go to GitHub then Releases then "Draft a new release"
2. Choose tag v0.7.0
...
3. Chart.js / KaTeX / Prism.js loaded from CDN -- first render waits
   on network
...
- Streaming parser for very large files (planned)
- Lazy-load CDN libraries (planned)
...
Check threshold in is_rtl_text() -- currently 30 percent RTL characters
required.
...
- Documentation: GUIDE.md, TUTORIAL.md
