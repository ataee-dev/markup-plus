...
Because CSS cascade rules apply, your custom rules override the base
styles.
...
All customizable variables are defined in `:root`. Override any of
them in your custom CSS file.
...
### Colors
    ...
    --code-accent        Accent inside code blocks                #0969da
...
Always try to solve styling with variables before writing custom
selectors.
...
A 20-line CSS file can completely transform the appearance. Start
small.
...
still overrides it. This lets you build themes that adapt to both
modes:
...
Create one CSS file and use it for all your documents:

    mup report.mup --css company-style.css
    mup proposal.mup --css company-style.css
    mup manual.mup --css company-style.css

### Test with Real Content

A theme that looks good with "Hello World" might break with real
content. Test with:
...
### Styles Break in Dark Mode

Cause: You used light colors that clash with dark background.

Fix: Test with --dark and adjust:
...
- GUIDE.md -- Full syntax reference
- DEVELOPER.md -- Architecture and contribution
